...
# NEW: import MelConfig to keep params consistent with dataset preprocessing
...
class BDVitsModel(nn.Module):
    ...
    which returns **waveform** directly. We derive a log-mel from the waveform using
    the SAME parameters as dataset mel extraction to improve alignment.
    """
    def __init__(self, base_model_name: str = "facebook/mms-tts-ben", n_mels: int = 80, sample_rate: int = 22050):
        ...
        # Keep mel params consistent with dataset default MelConfig
        ...
        # Minimal adapter kept for compatibility
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor = None):
        ...
        if waveform is not None:
            ...
            for b in range(waveform.size(0)):
                ...
                mel = self._waveform_to_log_mel(y)
                mels.append(mel)
            ...
            if mel is None:
                ...
                mel = hidden.transpose(1, 2)  # [B, n_mels, T] (roughly)

        logits_disc = self.disc(mel)  # [B, 2]
